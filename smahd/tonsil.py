...
def section_alignment_loss(z, section_ids):
    """Lightly align section-level latent moments without using annotations."""
    ...
    for section in unique_sections:
        values = normalized[section_ids == section]
        if values.shape[0] < 2:
            continue
        moments.append((values.mean(dim=0), values.std(dim=0, unbiased=False)))
    if len(moments) < 2:
        return z.new_zeros(())
    losses = []
    for left in range(len(moments)):
        for right in range(left + 1, len(moments)):
            mean_loss = F.smooth_l1_loss(moments[left][0], moments[right][0])
            scale_loss = F.smooth_l1_loss(moments[left][1], moments[right][1])
            losses.append(mean_loss + 0.25 * scale_loss)
    return torch.stack(losses).mean()


def cross_section_pair_loss(z, global_ids, pair_index):
    """Align label-free cross-section MNN pairs present in the current batch."""
    if global_ids is None or pair_index is None or pair_index.numel() == 0:
        return z.new_zeros(())
    max_id = int(max(global_ids.max().item(), pair_index.max().item())) + 1
    lookup = torch.full((max_id,), -1, dtype=torch.long, device=z.device)
    lookup[global_ids] = torch.arange(global_ids.numel(), device=z.device)
    source = lookup[pair_index[0]]
    target = lookup[pair_index[1]]
    keep = (source >= 0) & (target >= 0)
    if not torch.any(keep):
        return z.new_zeros(())
    normalized = F.normalize(z, p=2, dim=1)
    return (1.0 - (normalized[source[keep]] * normalized[target[keep]]).sum(dim=1)).mean()
...
def train_model(
    ...
    if node_sections is not None:
        data.section_id = node_sections.to(torch.long)
    ...
    pair_index = alignment_pairs.to(device) if alignment_pairs is not None else None
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in loader:
            ...
            alignment = (
                section_alignment_loss(z, getattr(batch, "section_id", None))
                if config.alignment_weight > 0
                else z.new_zeros(())
            )
            pair_alignment = (
                cross_section_pair_loss(
                    z,
                    getattr(batch, "global_id", None),
                    pair_index,
                )
                if config.pair_alignment_weight > 0
                else z.new_zeros(())
            )
            loss = (
                reconstruction
                ...
                + config.pair_alignment_weight * pair_alignment
            )
            loss.backward()
            ...
            total_pair_alignment += float(pair_alignment.detach().cpu())
            batches += 1
            updates += 1
        history.append({
            "epoch": epoch,
            ...
            "section_alignment_loss": total_alignment / max(1, batches),
            "cross_section_pair_loss": total_pair_alignment / max(1, batches),
        })
